...
class ShowBanPage extends AbstractAdminPage
{
	...
	function show()
	{
        ...
        $UserList		= $GLOBALS['DATABASE']->query("SELECT `username`, `id`, `bana` FROM ".USERS." WHERE `id` != 1 AND `authlevel` <= '".$USER['authlevel']."' AND `universe` = '".Universe::current()."' ".$WHEREBANA." ORDER BY ".$ORDER." ASC;");
        ...
        $Banneds		=0;
        $UserListBan	= $GLOBALS['DATABASE']->query("SELECT `username`, `id` FROM ".USERS." WHERE `bana` = '1' AND `universe` = '".Universe::current()."' ORDER BY ".$ORDER2." ASC;");
        ...
        $BANUSER				= $GLOBALS['DATABASE']->getFirstRow("SELECT b.theme, b.longer, u.id, u.urlaubs_modus, u.banaday FROM ".USERS." as u LEFT JOIN ".BANNED." as b ON u.`username` = b.`who` WHERE u.`username` = '".$GLOBALS['DATABASE']->sql_escape($Name)."' AND u.`universe` = '".Universe::current()."';");
        ...
        if(isset($_POST['panel']))
        {
            ...
            if ($BANUSER['banaday'] > TIMESTAMP)
            {
                ...
                $SQL     .= "`author` = '". $admin ."', ";
                $SQL     .= "`email` = '". $mail ."' ";
                $SQL     .= "WHERE `who2` = '".$Name."' AND `universe` = '".Universe::current()."';";
            ...
            $SQL    .= "WHERE ";
            $SQL    .= "`username` = '". $Name ."' AND `universe` = '".Universe::current()."';";
            $GLOBALS['DATABASE']->query($SQL);
            $this->printMessage($LNG['bo_the_player'].$Name.$LNG['bo_banned'], true, array('?page=ban', 3));
        } elseif(isset($_POST['unban_name'])) {
            $Name	= HTTP::_GP('unban_name', '', true);
            $GLOBALS['DATABASE']->query("UPDATE ".USERS." SET bana = '0', banaday = '0' WHERE username = '".$GLOBALS['DATABASE']->sql_escape($Name)."' AND `universe` = '".Universe::current()."';");
            #DELETE FROM ".BANNED." WHERE who = '".$GLOBALS['DATABASE']->sql_escape($Name)."' AND `universe` = '".Universe::current()."';
            $this->printMessage($LNG['bo_the_player2'].$Name.$LNG['bo_unbanned'], true, array('?page=ban', 3));
        }
        ...
        $this->display('page.ban.default.tpl');
	}
}
