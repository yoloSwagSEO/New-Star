...
class ShowMessageListPage extends AbstractAdminPage
{
	...
	function show()
	{
        ...
        if ($type != 100)
        {
            $MessageCount	= $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".MESSAGES.$countJoinSQL." WHERE message_type = ".$type." AND message_universe = ".Universe::current().$dateWhereSQL.$userWhereSQL.";");
        }
        else
        {
            $MessageCount	= $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".MESSAGES.$countJoinSQL." WHERE message_universe = ".Universe::current().$dateWhereSQL.$userWhereSQL.";");
        }
        ...
        if ($type == 100)
        {
            ...
            WHERE m.message_universe = ".Universe::current()."
            ".$dateWhereSQL."
            ".$userWhereSQL."
            ...
            LEFT JOIN ".USERS." as u ON m.message_owner = u.id
            LEFT JOIN ".USERS." as us ON m.message_sender = us.id
            WHERE m.message_type = ".$type." AND message_universe = ".Universe::current()."
            ".$dateWhereSQL."
            ".$userWhereSQL."
            ORDER BY message_time DESC, message_id DESC
            LIMIT ".$sqlLimit.";");
