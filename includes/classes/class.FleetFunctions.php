...
class FleetFunctions 
{
	...
	private static function GetShipSpeed($Ship, $Player)
	{
		...
		{
			case 1:
				$speed	= $pricelist[$Ship]['speed'] * (1 + ($Player['factor']['FlyTimeCom']) + ($Player['factor']['FlyTime']));
			break;
			case 2:
				$speed	= $pricelist[$Ship]['speed'] * (1 + ($Player['factor']['FlyTimeImp']) + ($Player['factor']['FlyTime']));
			break;
			case 3:
				$speed	= $pricelist[$Ship]['speed'] * (1 + ($Player['factor']['FlyTimeHyp']) + ($Player['factor']['FlyTime']));
			break;
			default:
				$speed	= 0;
			break;
